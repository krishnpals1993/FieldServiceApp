...
(function($ , undefined) {
	...
	sidebarMatchings.light = {
		...
		'content-white': true,
		'alert-1': 'This sidebar theme is more suitable for <a href="'+$('.sidebar .nav-link[href*="dashboard-3"]').attr('href')+'" class="btn btn-xs btn-outline-white px-1 border-0">Dashboard 3</a>',
	...
	*/
	...
	function applySidebarMatchingChanges($sidebar, theme) {
		...
		if( window.currentLayout == 1 ) {
			...
			if( changes['content-white'] ) {
				$('.main-container').addClass('bgc-white').find('.bcard').removeClass('bcard').addClass('ccard')				
			}
			else {
				$('.main-container').removeClass('bgc-white').find('.ccard').removeClass('ccard').addClass('bcard')
			}
	...
	*/
	...
	function BoxedLayout(box) {
		...
		if(box == 'none') {
			...
			if( window.currentLayout == 1 ) return;//not applicable to second layout
